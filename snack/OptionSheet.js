// Mirrors src/components/OptionSheet.tsx - if these two ever disagree, the
// .tsx file is the one that is wrong.
//
// One single-select sheet, used by every "pick one of a short list" control in
// the header - the sector filter and the number of groups. One tap opens it,
// one more picks and closes; no separate Apply, because unlike the window
// picker nothing here needs two coordinated choices before either is
// meaningful.
...
export function OptionSheet({ visible, title, options, selected, footnote, onClose, onSelect }) {
  ...
  return (
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      ...
      <View style={[s.sheet, { backgroundColor: colors.bg, borderColor: colors.border }]}>
        ...
        <ScrollView style={[s.list, { marginTop: space(3) }]} showsVerticalScrollIndicator={false}>
          {options.map((opt) => {
...
const s = StyleSheet.create({
  ...
  list: { flexGrow: 0 },
  row: {
    minHeight: ROW_HEIGHT, flexDirection: 'row', alignItems: 'center',
    justifyContent: 'space-between', paddingHorizontal: space(3),
    paddingVertical: space(1.5), borderRadius: radius.sm,
  },
