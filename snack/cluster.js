// Mirrors src/data/cluster.ts - if these two ever disagree, the .ts file is
...
// A cluster is { medoid, members, fit, weak, prefers }: the representative
// member, the matrix indices sorted best-fit first, each member's mean
// distance to the rest, whether it sits closer to another group (a negative
// silhouette - the visible price of the balance constraint), and which
// group's medoid that is.
...
function seedMedoids(n, k, d, rnd) {
 ...
 */
function constrainedAssign(n, medoids, lower, upper, d) {
...
function totalCost(assign, medoids, d) {
  let cost = 0;
 ...
 */
export function balancedKMedoids(n, k, d, sizeTolerance = 0.2) {
  const target = n / k;
  ...
  const clusters = buckets.map((members, c) => {
